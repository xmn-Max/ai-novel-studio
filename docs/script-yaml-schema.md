...
每种类型围绕一个核心看点进行 AI 分析，确保不同题材的剧本产出有本质差异：

| 类型 | 核心看点 | 角色提取重点 | 剧情分析重点 | 世界观分析重点 |
|------|---------|-------------|-------------|---------------|
| **叙事** | 看人生 | 人物命运轨迹、性格深度 | 命运转折、人生起伏 | 社会规则、人际关系网 |
| **玄幻** | 看成长 | 修炼层次、天赋潜力 | 升级打怪、突破瓶颈 | 修炼体系、实力等级 |
| **科幻** | 看思想 | 科学背景、理念立场 | 文明冲突、技术伦理 | 科技设定、未来社会结构 |
| **言情** | 看感情 | 情感纠葛、关系变化 | 感情发展、关系转折 | 情感规则（家族阻力） |
| **魔幻** | 看世界 | 种族身份、使命驱动 | 史诗冲突、善恶对决 | 魔法体系、种族设定 |
| **武侠** | 看侠义 | 武功流派、江湖立场 | 正邪对抗、侠义抉择 | 江湖势力、武学传承 |

### 世界观维度按类型适配

6 种类型的 6 个世界维度使用不同命名和含义，确保 AI 分析精准匹配题材：

| 维度 | 叙事 | 玄幻 | 科幻 | 言情 | 魔幻 | 武侠 |
|------|------|------|------|------|------|------|
| realms | 地点/环境 | 界域/地点 | 空间/星域 | 地点/空间 | 奇幻空间 | 江湖地点 |
| factions | 人物/群体 | 势力/宗门 | 群体/组织 | 人物/群体 | 势力/种族 | 势力/门派 |
| techniques | 能力/经历 | 功法/技能 | 科技/能力 | 情感/技能 | 魔法/技能 | 武功/技能 |
| items | 物品/符号 | 法宝/物品 | 物品/载具 | 物品/象征 | 神器/道具 | 兵器/道具 |
| timeline | 时间线 | 时间线 | 时间线 | 时间线 | 时间线 | 时间线 |
| rules | 规则/制度 | 规则 | 规则/原理 | 规则/社会约束 | 规则/魔法法则 | 规矩/侠义准则 |

实现方式：
- **后端**：`prompts.py` 中 `WORLD_FIELDS` 字典为每种类型存储 6 维度的完整描述，`pipeline.py` 运行时根据 `genre` 选择对应描述注入 `WORLD_BUILDING_PROMPT`
- **前端**：`WorldSection.tsx` 中 `GENRE_LABELS` 映射根据 `genre` prop 动态渲染对应标签
...
*文档版本: 4.2 · 最后更新: 2026-06-07 · 新增深度审阅系统和 Docker 部署支持*
