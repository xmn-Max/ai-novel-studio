...
=======
- **用户系统**：注册/登录后方可使用，用户信息存于 `users.json`，密码 SHA256 哈希
- **转换流水线（7 步）**：文本清洗 → 章节检测 → 角色提取 → 场景切分 → 剧本转换 → 主角验证 → Schema 校验
- **智能重生成**：验证不达标时不自动重试，改为向用户征求补充信息后完整重跑流水线
...
**系统默认类型**（`genres.json`，6 种，所有用户共享，不可编辑不可删除）：
武侠、玄幻、科幻、言情、叙事、魔幻

**用户自定义类型**（存储在 `users.json` 每个用户下，最多 10 个）：
>>>>>>> 63ce69f6a2c1e575377a37604d227d7053933c57
用户可增、删、改，仅自己可见
...
- **注册**：用户名 2-20 字符，密码 ≥ 4 字符
- **登录**：返回 UUID Token，存于 `localStorage`
- **鉴权**：所有 API 需 `Authorization: Bearer <token>` 头（SSE 通过 `?token=` 传参）
- **存储**：`users.json`，密码 SHA256 哈希，Token 每次登录时刷新
...
## 16. 与其他剧本格式的关系
=======
| 自定义类型 | `users.json` 中 per-user 存储 | 已实现，最多 10 个 |
| 多用户协作 | 引入数据库 + 权限系统 | 未来方向 |
