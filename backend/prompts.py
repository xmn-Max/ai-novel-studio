...
DEEP_REVIEW_PROMPT = """你是一个资深的剧本审阅顾问。用户对两个版本的剧本差异提出了补充意见，请结合原文内容，生成一个整合了两个版本优点的新剧本。
...
请根据以上信息，综合两个版本的优点并结合用户意见，重新生成完整的剧本场景列表 JSON：
{{
  "scenes": [
    {{
      "scene_id": 1,
...
注意：
1. 保持原文的核心剧情和角色设定不变
2. 在用户意见的指引下，综合两个版本中合理的部分
3. 对白自然有力，动作描述简洁准确
4. scene_id 从 1 开始按序号排列""" + COMMON_JSON_RULE
